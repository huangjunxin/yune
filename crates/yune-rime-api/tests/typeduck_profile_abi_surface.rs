...
use yune_rime_api::{
    rime_get_api, rime_get_typeduck_profile_api, RimeApi, RimeConfig, RimeTypeDuckProfileApi,
    FALSE, TRUE,
};

#[test]
fn default_rime_api_stays_upstream_sized_while_profile_is_larger() {
    ...
    assert_eq!(
        ...
        "profile accessor advertises the extended TypeDuck-profile table"
    );
    assert!(
        profile_api.upstream.data_size > default_api.data_size,
        "profile table must be opt-in and larger than the default upstream table"
    );

    let base = profile_api as *const RimeTypeDuckProfileApi as usize;
...
#[test]
fn typeduck_profile_append_string_slot_creates_and_extends_lists() {
    let profile_api = unsafe { &*rime_get_typeduck_profile_api() };
