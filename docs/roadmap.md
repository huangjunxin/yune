...
M45, WEB-01, M46, WEB-02, and WEB-03 are complete. M45 confirmed
...
lock the user-visible words down.
...
| Engine performance | M45 is complete as a partial native-engine follow-up to M44. Phase 0 selected `short-key-measured-no-go`, so no short-key engine implementation was retained. Final `hao` preserves the M44 pass at `24.267us` / `2.110x`, while `n` remains `68.900us` / `3.313x` and `ni` remains `49.450us` / `3.458x`. Track A steady resident rows meet the resident target (`87,498,752-98,684,928 B`), but the real cold-start peak remains `127,475,712 B`, so M45 records a standing peak-cost blocker. M46 confirms Track B still peaks at `504,627,200 B` with mostly unclassified process memory. | **Active: M47 iOS-budget native memory reduction.** RED-01 reduced isolated `jyut6ping3_mobile` keyboard-profile steady from `223.9 MB` to `169.2 MB`; RED-02 moved parsed prism storage to byte-backed runtime reads; RED-03 made compact lookup-record retention optional; RED-04 made the reverse/UI translator an optional keyboard-profile pack; RED-05 removed the deploy rebuild/high-water transient; RED-06 bounded the primary compact-table parse transient. The lean keyboard-profile Windows proxy is now `58.0 MB` steady WS, `23.3 MB` private, `16.0 MB` allocator live, and `62.9 MB` peak WS; the full mobile profile is still `195.2 MB` steady / `202.7 MB` peak, so do not treat the lean row as full product. Next branch is steady compact-table/code-index owners; target remains steady <= 48 MB / peak <= 64 MB. Keep `n`/`ni` as separate future Track A latency blockers. |
...
1. **M47 iOS-budget native memory reduction (active)** - the now-concrete memory
   ...
   Phase 0 attributed the old un-owned bucket with Windows `PrivateUsage`,
   working-set, and allocator-live evidence; RED-01 then made
   `dictionary_lookup_filter.lookup_records` optional for the keyboard profile,
   RED-02 moved parsed prism storage to byte-backed runtime reads, RED-03 made
   compact lookup-record retention optional for the keyboard-profile probe, and
   RED-04 made the reverse/UI translator an optional keyboard-profile pack,
   RED-05 removed the deploy rebuild/high-water transient, and RED-06 bounded
   the primary compact-table parse transient. The latest lean keyboard-profile
   `jyut6ping3_mobile` proxy is steady **58.0 MB** WS, **23.3 MB** private,
   **16.0 MB** allocator-live, and **62.9 MB** peak WS. The full mobile profile
   without opt-outs is still **195.2 MB** steady / **202.7 MB** peak, so the
   lean row is not a full-product claim. The next reduction branch is steady
   compact-table/code-index owners after `create_session`. Plan:
   [`plans/active/m47-plan-ios-budget-native-memory-reduction.md`](./plans/active/m47-plan-ios-budget-native-memory-reduction.md);
...
| M47 | Active (post-RED-06 create-session transient reduced) | iOS-budget native memory reduction: drive single-active-schema native working set under an iOS keyboard-extension budget (steady <= 48 MB, peak <= 64 MB; stretch 48 MB) as portable engine work. Phase 0 Windows `PrivateUsage`/working-set evidence attributed the old un-owned bucket as mostly live retained heap / process-private memory, not allocator-retained-free. RED-01 added an explicit keyboard-profile gate for `dictionary_lookup_filter.lookup_records`, moving isolated `jyut6ping3_mobile` steady `223.9 -> 169.2 MB`, private `202.2 -> 147.7 MB`, allocator-live `155.0 -> 104.9 MB`, peak `231.6 -> 217.3 MB`. RED-02 moved parsed prism storage to byte-backed runtime reads, moving steady `169.1 -> 138.1 MB`, private `146.6 -> 101.8 MB`, allocator-live `104.9 -> 66.6 MB`, and peak `217.2 -> 172.1 MB`. RED-03 made compact lookup-record retention optional for the keyboard-profile probe, moving steady `137.8 -> 69.4 MB`, private `102.8 -> 30.2 MB`, allocator-live `66.6 -> 20.4 MB`, and peak `172.1 -> 80.8 MB`. RED-04 made the reverse/UI translator an optional keyboard-profile pack, moving steady `69.2 -> 58.5 MB`, private `29.7 -> 23.3 MB`, allocator-live `20.4 -> 16.0 MB`, while peak stayed `80.7 -> 81.0 MB`. RED-05 removed the deploy rebuild/high-water transient, moving steady `58.5 -> 56.9 MB`, after-deploy peak `81.0 -> 12.4 MB`, and overall peak `81.0 -> 78.4 MB`. RED-06 dropped reverse parse scratch before compact-table parse, moving fresh lean keyboard-profile peak `79.6 -> 62.9 MB` and allocator high-water `35.4 -> 22.7 MB`, while steady stayed `58.1 -> 58.0 MB`. Next branch: steady compact-table/code-index owners, especially `normal_codes`. Evidence: [`reports/evidence/m47-ios-budget-native-memory-reduction-red06-2026-06-29/`](./reports/evidence/m47-ios-budget-native-memory-reduction-red06-2026-06-29/). Plan: [`plans/active/m47-plan-ios-budget-native-memory-reduction.md`](./plans/active/m47-plan-ios-budget-native-memory-reduction.md). |
