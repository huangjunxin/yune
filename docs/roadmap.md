...
1. **M47 iOS-budget native memory reduction (active)** - the now-concrete memory
   ...
   and RED-02 moved parsed prism storage to byte-backed runtime reads. The latest
   isolated `jyut6ping3_mobile` proxy is steady **138.1 MB** WS, **101.8 MB**
   private, **66.6 MB** allocator-live, and **172.1 MB** peak WS. The next
   reduction branch is compact `lookup_records`, before reverse/UI lazy loading
   unless that branch proves the UI path must be deferred. Plan:
   [`plans/active/m47-plan-ios-budget-native-memory-reduction.md`](./plans/active/m47-plan-ios-budget-native-memory-reduction.md);
