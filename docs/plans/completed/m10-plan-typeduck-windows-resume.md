...
> **Status:** Finished - T1/T2/T3 verified - **Milestone:** M10 (TypeDuck-Windows native backend) - **Updated:** 2026-06-21 - **Type:** archived execution plan
...
- [ ] If all gates pass, mark M10 complete in `docs/roadmap.md` and move the plan under `docs/plans/completed/`.
...
- 2026-06-21: M10 T3 completed. Root cause of the final blocker was Yune's fresh session staying on placeholder `default` instead of selecting the first deployed schema from `default.yaml` like librime's default engine/switcher path, so TypeDuck-Windows reached schema-specific settings with a non-schema id. Yune now applies the first deployed schema on session creation when a deployed schema list exists, with regression coverage in `fresh_session_uses_first_deployed_schema_for_schema_specific_settings`. After repackaging and rebuilding stock TypeDuck-Windows server/test artifacts, stock `TypeDuckServer.exe` plus stock `TestTypeDuckIPC.exe /console` returned exit code `0`, sent `ngohaig` key events, and recorded `status.schema_id=jyut6ping3`, `ctx.preedit=ngohaig`, and candidate data. Tracked evidence: `docs/plans/completed/m10-evidence/t3-stock-real-server/`. This is a stock server/client IPC smoke; interactive TSF typing and visible candidate-window rendering are deferred to Phase 2. No default ABI structs, default `rime_get_api()` layout, or TypeDuck profile ABI slots changed.
