...
- Schema pipeline compatibility: focused librime-style subsets now cover
  ...
  horizontal linear, vertical stacked, and vertical linear candidate lists. The
  current shape coverage formats committed ASCII text under `full_shape` and
...
- The schema pipeline is still a subset. The current focused coverage now
  ...
  deeper `selector` custom key-binding and navigator fallback interaction
  behavior beyond the current focused raw-tag and layout coverage,
