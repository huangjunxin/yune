...
| M33-PERF-REQ-01 | M33 | Draft - fresh native and cross-engine baselines recaptured before optimization claims |
| M33-PERF-REQ-02 | M33 | Draft - comparison fairness enforced: same schema/assets/modules and no luna+stroke Yune row compared against luna-only librime |
| M33-PERF-REQ-03 | M33 | Draft - lazy reverse-lookup loading removes the eager `stroke` startup/session/resident asymmetry while preserving first-use reverse lookup behavior |
| M33-PERF-REQ-04 | M33 | Draft - build-once schema cache reduces re-select/session cost with byte-identical candidates and correct deploy invalidation |
| M33-PERF-REQ-05 | M33 | Draft - lazy spelling-algebra prism lookup proceeds only after a byte-identical feasibility spike; otherwise it is explicitly deferred |
| M33-PERF-REQ-06 | M33 | Draft - mmap compiled artifacts proceeds only with Windows lifetime/deploy safety evidence and byte-identical behavior |
| M33-PERF-REQ-07 | M33 | Draft - public-facing performance copy and README chart are refreshed only after the fairness-controlled rerun |
| M33-PERF-REQ-08 | M33 | Draft - full compatibility, ABI, benchmark, runtime, TypeDuck-Web, and diff gates pass |
