...
_Requirements defined: 2026-04-28_ _Last updated: 2026-06-24 - M35 is complete as the compact table+prism runtime storage milestone. It captured fresh native and fair cross-engine baselines, replaced heap-slice lookup with candidate views, added compact table exact/prefix/all-code lookup, integrated prism canonical-code discovery with table payload lookup, enabled compact storage for safe upstream `luna_pinyin` without retained heap `entries_by_code`, kept TypeDuck `jyut6ping3` on documented heap fallback, and updated reports with native/fair/memory/browser-delivery separation. Native `luna_pinyin_zhongguo_full_abi` improved `14,759.755us` -> `1,527.055us`; `spelling_algebra_expand` dropped `148,570.200us` / `17,784,832` bytes to `122.200us` / `0` bytes. Mmap/borrowed storage is closed by no-go for M35 and deferred to a separate measured design because fair-harness whole-process peak remains about `182 MB`. M34 remains closed as the bounded lazy candidate pipeline and lookup-abstraction milestone. M31 remains drafted with public-demo gates. M33 engine native lookup performance remains complete with eight requirements closed. P2-WIN-02 remains complete as the Yune boundary fix for TypeDuck Windows rich comment bytes; the remaining Notepad raw-ASCII behavior belongs to non-Yune TSF input-delivery/frontend-shell work. M30, M29, M28 follow-up, M28, M27, M26, M25, M24, M19, M23, M18, M22, M21, M20, and M10 remain complete as previously recorded._
