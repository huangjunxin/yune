...
| M36-PERF-REQ-01 | M36 | Draft - native in-process harness replaces managed cross-engine measurement and records Track A (`luna_pinyin` Yune vs librime) separately from Track B (`jyut6ping3` Yune before/after), with startup, per-key, resident, and peak memory rows |
| M36-PERF-REQ-02 | M36 | Draft - every optimization phase is greenlit only after profiler attribution tags the owner as product or comparison and meets the plan's measured owner threshold; comparison-only wins are never presented as product wins |
| M36-PERF-REQ-03 | M36 | Draft - product path verification records whether `jyut6ping3` engages compiled table/prism assets or falls back to source YAML, and attributes `rime:init` time before Phase 2 claims |
| M36-PERF-REQ-04 | M36 | Draft - free browser/WASM wins, including measured `INITIAL_MEMORY` sizing and gating comparison-only assets out of the product bundle, land only with browser smoke and no hidden engine-latency claim |
| M36-PERF-REQ-05 | M36 | Draft - product storage interning uses a byte arena / `StringId` design on the measured product storage path, preserving API-boundary owned `Candidate` values and carrying the dedup benefit through any later compact-product path |
| M36-PERF-REQ-06 | M36 | Draft - TypeDuck product compiled-table support lands through `rsmarisa` marisa string-table consumption for the actual `jyut6ping3` and `jyut6ping3_scolar` product blobs, noting `rsmarisa`'s marisa `0.3.1` compatibility and verifying real-blob `str_id -> UTF-8` parity plus nested/multi-trie shape where present; native mmap and WASM byte-backed loading are measured separately; if `rsmarisa` fails byte-parity/MSRV/WASM/performance gates, fallback Yune-readable no-marisa re-emitted assets verify `i32@60 == 0` / `u32@64 == 0`; either path has no runtime `SourceFallback` |
| M36-PERF-REQ-07 | M36 | Draft - product canonical table via prism stops discarding the prism payload on the product translator path, avoids eager spelling-algebra expansion, and stays byte-identical for rich comments, lookup records, tolerance/correction, partial selection, and M28 long composition |
| M36-PERF-REQ-08 | M36 | Draft - bounded/lazy candidate pipeline generalizes to the TypeDuck product path only where first-page, paging, filters/rankers, context snapshots, and userdb behavior remain byte-identical; unsafe cases keep eager fallback |
| M36-PERF-REQ-09 | M36 | Draft - closeout reports native vs browser, product vs comparison, memory vs latency, and win vs no-go separately; M36 reruns the full final benchmark comparison against the Phase 0 baseline and current librime oracle where applicable, and does not claim "matched librime" or "faster than librime" unless the measured data actually proves it |
| M36-PERF-REQ-10 | M36 | Draft - final gates pass for Rust fmt/clippy/tests, focused upstream and TypeDuck parity, workspace tests, final native benchmarks, report/visualization regeneration, diff checks, and runtime/browser/patch checks whenever runtime-visible files change |
...
- M36 product-path engine optimization requirements: 10 total, 0 complete, 10 draft
- Mapped to phases: 219
- Unmapped: 0

---

_Requirements defined: 2026-04-28_ _Last updated: 2026-06-24 - M36 is drafted as the active product-path engine optimization milestone after M35. Its ten draft requirements keep fair `luna_pinyin` comparison separate from TypeDuck `jyut6ping3` product before/after evidence, require native in-process measurement, make `rsmarisa` the first compiled-table path for product marisa string tables, require the `rsmarisa` spike to use the actual product blobs and nested/multi-trie evidence rather than synthetic fixtures, keep no-marisa re-emitted assets as the fallback, require a final full benchmark comparison plus regenerated performance-report visualizations before closeout, and preserve honest reporting/no-go discipline. M35 remains complete as the compact table+prism runtime storage milestone: native `luna_pinyin_zhongguo_full_abi` improved `14,759.755us` -> `1,527.055us`; `spelling_algebra_expand` dropped `148,570.200us` / `17,784,832` bytes to `122.200us` / `0` bytes; TypeDuck `jyut6ping3` stayed on the documented heap fallback. M31 remains drafted with public-demo gates and should serialize engine/schema work behind M36. M33, M34, P2-WIN-02, M30, M29, M28 follow-up, M28, M27, M26, M25, M24, M19, M23, M18, M22, M21, M20, and M10 remain complete as previously recorded._
